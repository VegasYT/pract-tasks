...
def run_multiprocess() -> float:
    ...
    elapsed = time.perf_counter() - start
    print(f"[Многопроцессорный] Время: {elapsed:.2f} сек  (ядер CPU: {cpu_count})")
    return elapsed
...
if __name__ == "__main__":
    ...
    print(f"  Многопоточный:     {t_threaded:.2f} сек  (ускорение x{t_sync / t_threaded:.2f})")
    print(f"  Многопроцессорный: {t_multiproc:.2f} сек  (ускорение x{t_sync / t_multiproc:.2f})")